#!/usr/bin/env python3
"""Rewrite case-law-research.ts with properly escaped strings."""
import re

content = r"""// Case Law Research Agent
...
// Court type to keyword mapping (module-level constant for performance)
const COURT_TYPE_KEYWORDS: Record<CourtType, string[]> = {
...
function matchCaseLawEntries(query: string, courtType: CourtType): CaseLawEntry[] {
  ...
  const relevantCourts = COURT_TYPE_KEYWORDS[courtType]

  // Pre-filter: only entries whose legalIssue or jurisdiction mentions relevant court keywords
  return CASE_DATABASE
    .filter(entry => {
      const jurisdictionLower = entry.jurisdiction.toLowerCase()
      const issueLower = entry.legalIssue.toLowerCase()
      return relevantCourts.some(c => jurisdictionLower.includes(c) || issueLower.includes(c))
    })
    .map(entry => {
      let score = 0

      // Keyword matching on pre-filtered results
      for (const kw of keywords) {
        if (issueLower.includes(kw)) score += 3
...
export class CaseLawResearchAgent {
  ...
  public getRecentUpdates(courtType: CourtType, limit = 5): CaseLawEntry[] {
    ...
    return CASE_DATABASE
      .filter(entry => {
        const issueLower = entry.legalIssue.toLowerCase()
        return relevantCourts.some(c => issueLower.includes(c))
      })
...
# Fix the \\t in the code (should be whitespace split, not \t literal)
content = content.replace(r'\t', ' ')

with open('src/lib/agents/legal-research/case-law-research.ts', 'w', encoding='utf-8') as f:
    f.write(content)

print('case-law-research.ts rewritten with proper escaping')
"""

with open('fix_strings.py', 'w') as f:
    f.write(content)
print('Script written to fix_strings.py')