...
// Mock the document generator — route layer only (not unit-testing the generator)
vi.mock('@/lib/agents/legal-research/document-generator', () => ({
  ...
  // Re-export types so route can import them from here
  CourtType: {} as any,
...
function createMockResponse() {
  return vi.fn((data: object, _opts?: object) => data) as unknown as typeof mockNextResponseJson
}
...
describe('Legal Document Generation API', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET handler', () => {
    it('returns 401 when not authenticated', async () => {
      ...
      const request = createMockRequest('http://localhost/api/legal/documents')
    ...
    it('returns all templates when action=list-templates (no filter)', async () => {
      ...
      ;(legalDocumentGenerator.listTemplates as ReturnType<typeof vi.fn>).mockReturnValue([
        { type: 'family-intake-form', title: 'Family Law Intake Form', description: 'Comprehensive intake form', courtType: 'family', jurisdiction: 'State Family Court', estimatedTime: '25-30 min', sections: [{ fields: [{ id: 'a', label: 'A', type: 'text', required: true }] }] },
      ])
      mockNextResponseJson.mockImplementation((data: object) => data)

      const { GET } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents?action=list-templates')
    ...
    it('returns IP templates when list-templates filtered by courtType=ip', async () => {
      ...
      const { GET } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents?action=list-templates&courtType=ip')
    ...
    it('returns template when action=get-template with valid type', async () => {
      ...
      ;(legalDocumentGenerator.getTemplate as ReturnType<typeof vi.fn>).mockReturnValue({
        ...
        description: 'Petition for child custody',
        courtType: 'family',
      ...
      const { GET } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents?action=get-template&type=custody-petition')
    ...
    it('returns 400 when get-template missing type parameter', async () => {
      ...
      const { GET } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents?action=get-template')
    ...
    it('returns 404 when get-template type not found', async () => {
      ...
      const { GET } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents?action=get-template&type=nonexistent-type')
    ...
    it('returns catalog with family, bankruptcy, and ip keys when action=catalog', async () => {
      const { legalDocumentGenerator } = await import('@/lib/agents/legal-research/document-generator')
      ...
      const { GET } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents?action=catalog')
    ...
    it('returns 400 for unknown action', async () => {
      ...
      const { GET } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents?action=unknown-action')
  ...
  describe('POST handler', () => {
    it('returns 401 when not authenticated', async () => {
      ...
      const request = createMockRequest('http://localhost/api/legal/documents', { action: 'generate' })
        ...
        sections: [
          {
            ...
            fields: [
              { id: 'fullName', label: 'Full Legal Name', value: 'John Doe', filled: true },
            ],
          },
        ],
    ...
    it('returns 400 when preview missing documentType', async () => {
      ...
      const { POST } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents', { action: 'preview' })
    ...
    it('returns 404 when preview template not found', async () => {
      const { legalDocumentGenerator } = await import('@/lib/agents/legal-research/document-generator')
      ;(legalDocumentGenerator.getTemplate as ReturnType<typeof vi.fn>).mockReturnValue(undefined)
      mockNextResponseJson.mockImplementation((data: object) => data)
      ...
      expect(result).toEqual({ success: false, error: "Invalid documentType" })
    })
    ...
    it('returns 400 when list-by-court missing courtType', async () => {
      ...
      const { POST } = await import('@/app/api/legal/documents/route')
      const request = createMockRequest('http://localhost/api/legal/documents', { action: 'list-by-court' })
