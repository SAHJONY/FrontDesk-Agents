...
let defaultModel = 'google/gemma-2-2b-it' // Fast, verified working model
...
/**
 * Reset model failure tracking (call at start of new task)
 */
...
/**
 * Get the next available model, skipping failed ones
 * This implements autonomous fallback within NVIDIA models
 */
export function getNextAvailableModel(preferredCategory?: keyof typeof MODEL_CATEGORIES): string | null {
  // Start with verified models in preferred category, then fallback to all verified
  const tryModels = preferredCategory 
    ? [...getModelsByCategory(preferredCategory), ...getVerifiedModels()]
    : getVerifiedModels()
  
  // Remove duplicates and failed models
...
/**
 * Autonomous completion with automatic model fallback
...
export async function completeWithAutonomousFallback(
  ...
  // Try up to maxRetries different models
...
/**
 * Quick test if a model is currently accessible
 */
...
/**
 * Chat with JSON mode (structured outputs)
 */
export async function chatJSON(
  ...
  const response = await chat(messages, {
    ...options,
    // NVIDIA may support response_format for JSON mode
  })
...
/**
 * Analyze customer sentiment using NVIDIA AI
 */
...
/**
 * Generate call summary using NVIDIA AI
 */
...
/**
 * Classify call intent using NVIDIA AI
 */
...
/**
 * Autonomous decision making for call flow using NVIDIA AI
 */
...
// Model config with reasoning flag for models that output internal thoughts
export interface NVIDIAModelConfig {
  ...
  verified: boolean
  reasoning?: boolean // True for models that output internal reasoning thoughts
}

// All available NVIDIA models organized by category
export const NVIDIA_MODELS: NVIDIAModelConfig[] = [
  // QUICK TEST - Small, fast models that work reliably
  ...
  // LARGER MODELS - More capable but slower
...
// Get only verified working models
...
// Keep backward compatibility - default export with array for dropdown
