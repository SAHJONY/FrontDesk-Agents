  ...
  try {
    ...
    if (action === 'catalog') {
      ...
      return NextResponse.json({
        ...
        totalDocuments: catalog.family.length + catalog.bankruptcy.length + catalog.ip.length + catalog.immigration.length,
      })
    }
    ...
    if (action === 'list-by-court') {
      ...
      if (!(ALLOWED_COURT_TYPES as readonly string[]).includes(courtType)) {
        return NextResponse.json({ success: false, error: "courtType must be 'family', 'bankruptcy', 'immigration', or 'ip'" }, { status: 400 })
      }
