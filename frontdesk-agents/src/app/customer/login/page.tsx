...
const translations = {
  en: {
    title: 'Welcome Back',
    subtitle: 'Sign in to your FrontDesk Agents AI account',
  ...
  es: {
    ...
    subtitle: 'Inicia sesión en tu cuenta de FrontDesk Agents AI',
  ...
  fr: {
    title: 'Bon Retour',
    subtitle: 'Connectez-vous à votre compte FrontDesk Agents AI',
  ...
  zh: {
    title: '欢迎回来',
    subtitle: '登录您的FrontDesk Agents AI账户',
    emailAddress: '电子邮件地址',
    emailPlaceholder: 'you@company.com',
    password: '密码',
  ...
  hi: {
    ...
    subtitle: 'अपने FrontDesk Agents AI खाते में साइन इन करें',
  ...
  ar: {
    title: 'مرحبًا بعودتك',
    subtitle: 'سجل دخولك إلى حساب FrontDesk Agents AI الخاص بك',
  ...
  pt: {
    ...
    subtitle: 'Entre na sua conta FrontDesk Agents AI',
  ...
  ko: {
    ...
    subtitle: 'FrontDesk Agents AI 계정에 로그인하세요',
    emailAddress: '이메일 주소',
    emailPlaceholder: 'you@company.com',
    password: '비밀번호',
  ...
  ja: {
    title: 'おかえりなさい',
    subtitle: 'FrontDesk Agents AIアカウントにログイン',
  ...
  vi: {
    ...
    subtitle: 'Đăng nhập vào tài khoản FrontDesk Agents AI của bạn',
  ...
  tl: {
    ...
    subtitle: 'Mag-sign in sa iyong FrontDesk Agents AI account',
    emailAddress: 'Email Address',
  ...
  de: {
    ...
    subtitle: 'Melden Sie sich bei Ihrem FrontDesk Agents AI-Konto an',
  ...
  it: {
    title: 'Bentornato',
    subtitle: 'Accedi al tuo account FrontDesk Agents AI',
  ...
  ru: {
    ...
    subtitle: 'Войдите в свой аккаунт FrontDesk Agents AI',
    ...
    <div className={`min-h-screen transition-colors duration-500 ${isDark ? 'bg-black' : 'bg-gray-50'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <main className='min-h-screen flex items-center justify-center px-4 pt-24'>
        <div className='w-full max-w-md'>
          ...
          >
            <h1 className={`text-4xl font-bold mb-4 font-display ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {t.title}
            </h1>
          ...
          >
            <form onSubmit={handleLogin} className='space-y-6'>
              {/* Error Message */}
              {error && (
                <div className={`flex items-center gap-2 p-4 rounded-xl ${isDark ? 'bg-cinematic-red/10 border border-cinematic-red/20' : 'bg-red-50 border border-red-200'}`}>
                  <AlertCircle className={`w-5 h-5 ${isDark ? 'text-cinematic-red' : 'text-red-600'}`} />
                  <span className={`text-sm ${isDark ? 'text-cinematic-red' : 'text-red-600'}`}>{error}</span>
                </div>
              )}
            ...
            <p className={`text-center text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {t.dontHaveAccount}{' '}
              <Link href='/customer/signup' className='text-green-500 hover:text-emerald-400 font-medium'>
                {t.createOne}
              </Link>
            </p>
          </motion.div>
