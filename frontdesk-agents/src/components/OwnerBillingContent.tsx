...
interface OwnerBillingContentProps {
  /** When true, hides the header (back link, title, refresh/export buttons) — for embedding in dashboard tab */
  embedded?: boolean
}
...
export default function OwnerBillingContent({ embedded = false }: OwnerBillingContentProps) {
  ...
  return (
    <div className="min-h-screen bg-black">
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header — hidden when embedded in dashboard tab */}
        {!embedded && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          >
            ...
            <p className="text-2xl font-bold text-emerald-400">{formatCurrency(totalRevenue, 'usd')}</p>
            <p className="text-xs text-gray-500 mt-1">{records.filter(r => r.status === 'succeeded').length} successful payments</p>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-gray-500 uppercase tracking-wider">Customers</span>
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <AlertCircle className="w-4 h-4 text-cinematic-red" />
            ...
            <p className="text-xs text-gray-500 mt-1">
              {failedCount > 0 ? `${((failedCount / records.length) * 100).toFixed(1)}% failure rate` : 'No failures'}
            </p>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <Search className="w-4 h-4 text-aurora-cyan" />
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-aurora-cyan/40 transition-all"
            />
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-[140px] pl-8 pr-2 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-aurora-cyan/40 transition-all [color-scheme:dark]"
                title="Start date"
              />
            </div>
            ...
            {(['all', 'succeeded', 'failed', 'refunded', 'pending'] as const).map((s) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  statusFilter === s
                    ? 'bg-aurora-cyan text-white animate-glow'
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-gray-400 hover:text-gray-300 transition-colors"
          >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className="text-sm font-medium text-white truncate">{record.description}</span>
                        <span className={`inline-block text-xs px-2 py-0.5 rounded-full border capitalize ${
                          record.status === 'succeeded' ? 'text-green-400 border-green-500/20 bg-green-500/5' :
                          record.status === 'failed' ? 'text-cinematic-red border-red-500/20 bg-red-500/5' :
                          record.status === 'refunded' ? 'text-hollywood-gold border-yellow-500/20 bg-yellow-500/5' :
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500">{record.billing_reason || 'General'}</p>
                    </div>
