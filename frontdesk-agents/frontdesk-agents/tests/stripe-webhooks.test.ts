...
/** Factory for a minimal Stripe Event object (only the properties handlers touch). */
...
/** Factory for a customer record returned by the supabase helpers. */
...
/** Factory for a billing record returned by createBillingRecord. */
...
beforeEach(() => {
  vi.clearAllMocks()
  // Set env vars that the handlers check at runtime
...
describe('customer.subscription.updated', () => {
  ...
  it('maps trialing status to active', async () => {
    mockGetCustomerByStripeSubscriptionId.mockResolvedValueOnce(
      createCustomer({ plan: 'premium', status: 'active' })
    )
    mockUpdateCustomer.mockResolvedValueOnce(
      createCustomer({ plan: 'premium', status: 'active' })
    )
  ...
  it('falls back to customer.plan when subscription metadata has no plan', async () => {
    ...
    // Should retain the customer's existing plan (enterprise) since subscription metadata has no plan
    expect(mockUpdateCustomer).toHaveBeenCalledWith(
      'cust-001',
...
describe('invoice.payment_succeeded', () => {
  it('records billing history entry', async () => {
    ...
    // Customer was already active — should NOT be updated
    expect(mockUpdateCustomer).not.toHaveBeenCalled()
  })
